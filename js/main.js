import { M as Modal } from "./bootstrap.js";
(function polyfill() {
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.querySelectorAll("[data-phone-input]").forEach((input) => {
    ...
    input.addEventListener("input", () => {
      input.value = formatRuPhone(getPhoneDigits(input.value));
    });
    input.addEventListener("paste", (event) => {
      var _a;
      event.preventDefault();
      const pasted = ((_a = event.clipboardData) == null ? void 0 : _a.getData("text")) ?? "";
      input.value = formatRuPhone(getPhoneDigits(pasted));
    });
  ...
  if (bookingModalEl) {
    ...
    const bookingModalContentEl = bookingModalEl.querySelector(".modal-content");
    const bookingModalTitleEl = bookingModalEl.querySelector("#bookingModalLabel");
    const bookingModalImageEl = bookingModalEl.querySelector("#bookingModalImage");
    const bookingModalForm = bookingModalEl.querySelector("#bookingModalForm");
    const bookingRequiredError = bookingModalEl.querySelector("#bookingRequiredError");
    const bookingNameInput = bookingModalEl.querySelector("#bookingNameInput");
    const bookingPhoneInput = bookingModalEl.querySelector("#bookingPhoneInput");
    const bookingRequiredInputs = [bookingNameInput, bookingPhoneInput];
    const bookingModalContentByType = {
      master: {
        title: "Запись на мастер-класс",
        image: "images/modal_image.png"
      },
      holiday: {
        title: "Организуем праздник для Вас!",
        image: "images/modal_image_3.png"
      },
      default: {
        title: "Запишитесь на занятие!",
        image: "images/modal_image_2.png"
      }
    };
    const applyBookingModalContent = (type) => {
      const config = bookingModalContentByType[type] ?? bookingModalContentByType.default;
      if (bookingModalTitleEl) {
        bookingModalTitleEl.textContent = config.title;
      }
      if (bookingModalImageEl) {
        bookingModalImageEl.setAttribute("src", config.image);
      }
    };
    applyBookingModalContent("default");
    if (bookingModalForm && bookingRequiredError && !bookingRequiredInputs.some((input) => !input)) {
      bookingRequiredInputs.forEach((input) => {
        input.addEventListener("focus", () => {
          input.classList.remove("booking-modal__input-error");
          bookingRequiredError.style.display = "none";
        });
      });
      bookingModalForm.addEventListener("submit", (event) => {
        ...
        bookingRequiredInputs.forEach((input) => {
          if (!input.value.trim()) {
            input.classList.add("booking-modal__input-error");
            hasErrors = true;
          }
        });
        if (hasErrors) {
          bookingRequiredError.style.display = "block";
          return;
        }
        if (bookingModalContentEl) {
          bookingModalContentEl.innerHTML = "<button type='button' class='btn-close booking-modal__close' data-bs-dismiss='modal' aria-label='Закрыть'></button><div class='booking-modal__success'><h3 class='booking-modal__success-title'>Ваши данные отправлены!</h3><p class='booking-modal__success-text'>Мы с вами свяжемся в ближайшее время</p></div>";
        }
        setTimeout(() => {
          bookingModal.hide();
        }, 7e3);
    ...
    bookingModalEl.addEventListener("hide.bs.modal", () => {
      const active = document.activeElement;
      if (active && bookingModalEl.contains(active)) {
        active.blur();
      }
    });
    document.querySelectorAll("[data-open-modal]").forEach((btn) => {
      btn.addEventListener("click", () => {
        applyBookingModalContent(btn.dataset.modalType ?? "default");
        bookingModal.show();
      });
    });
  }
  ...
  tinySliderExports.tns({
    ...
    nav: false,
    responsive: {
      0: { fixedWidth: 280, gutter: 8 },
      480: { fixedWidth: 360, gutter: 10 },
      768: { fixedWidth: 600, gutter: 10 },
      1100: { fixedWidth: 900, gutter: 10 }
    }
  });
});
